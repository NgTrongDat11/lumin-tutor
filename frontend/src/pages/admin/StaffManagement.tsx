...
function ActionMenu({ account, onToggle, onReset, loading }: {
...
function StaffCard({ account, onToggle, onReset, loading }: {
  account: AdminStaffResponse;
  onToggle: () => void;
  onReset: () => void;
  loading: boolean;
  ...
  return (
    <article className={`rounded-lg border p-4 transition-all ${suspended ? 'border-danger-200 bg-danger-50/20' : 'border-border-light bg-white hover:shadow-sm'}`}>
      <div className="flex items-start justify-between gap-3">
        ...
        <ActionMenu account={account} onToggle={onToggle} onReset={onReset} loading={loading} />
      </div>
