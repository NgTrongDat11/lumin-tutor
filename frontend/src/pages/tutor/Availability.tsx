...
export default function TutorAvailability() {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Xoá lịch rảnh này?')) return;
      ...
      {avails.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[2, 3, 4, 5, 6, 7, 1].map((day) => {
            ...
            return (
              <Card key={day}>
                <h3 className="font-semibold text-text-primary mb-3">{dayNames[day]}</h3>
  ...
  return (
    <Modal open={open} onClose={onClose} title="Thêm lịch rảnh" footer={<><Button variant="outline" onClick={onClose}>Huỷ</Button><Button loading={loading} onClick={(e) => handleSubmit(e as unknown as FormEvent)}>Thêm</Button></>}>
      <form onSubmit={handleSubmit} className="space-y-4">
        <Select label="Thứ" options={[2,3,4,5,6,7,1].map((d) => ({ value: String(d), label: dayNames[d] }))} value={String(form.day_of_week)} onChange={(e) => setForm((f) => ({ ...f, day_of_week: Number(e.target.value) }))} />
