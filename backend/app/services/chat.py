...
1. **GỢI Ý từ thuật toán**: Khi sinh viên có nhu cầu học, hãy **ưu tiên sử dụng `algorithm_recommendations`** — đây là kết quả từ thuật toán gợi ý thông minh (hybrid content-based + collaborative filtering) đã tính điểm phù hợp. Nêu rõ tên gia sư/lớp, điểm phù hợp (score), và lý do (reasons).
...
Hệ thống Lumin sử dụng thuật toán gợi ý hybrid (content-based + collaborative filtering) để tính điểm phù hợp cho mỗi gia sư/lớp dựa trên:
- Môn học phù hợp (30 điểm)
- Cấp độ lớp phù hợp (15 điểm)
- Học phí trong ngân sách (15 điểm)
- Lịch học khớp (15 điểm)
- Hình thức dạy phù hợp (10 điểm)
- Đánh giá trung bình (10 điểm)
- Kinh nghiệm giảng dạy (5 điểm)
Kết quả được sắp xếp theo tổng điểm từ cao đến thấp.
...
async def build_student_context(student_id: int, db: AsyncSession) -> str:
    ...
    classes = await _get_open_group_classes(db)
...
async def compute_and_save_recommendation_snapshot(
    need: LearningNeed, db: AsyncSession
) -> None:
...
async def _get_open_group_classes(db: AsyncSession) -> list[dict[str, Any]]:
    result = await db.execute(
...
async def _get_available_tutors(db: AsyncSession) -> list[dict[str, Any]]:
    ...
    for tutor in tutors:
        ...
        availability = [
            {
                "day": day_names[avail.day_of_week] if 0 <= avail.day_of_week < 7 else str(avail.day_of_week),
