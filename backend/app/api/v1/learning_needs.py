"""Learning needs API — create, list, get, fulfill."""
...
from app.schemas.learning_need import LearningNeedCreate, LearningNeedResponse
...
async def list_my_learning_needs(
    ...
    result = await db.execute(
        select(LearningNeed)
        .options(selectinload(LearningNeed.schedules))
        .where(LearningNeed.student_account_id == current_user.id)
...
async def get_learning_need(
    ...
    result = await db.execute(
        select(LearningNeed)
        .options(selectinload(LearningNeed.schedules))
        .where(LearningNeed.id == need_id, LearningNeed.student_account_id == current_user.id)
    )
